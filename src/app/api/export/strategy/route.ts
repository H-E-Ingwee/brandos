...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .cover { background: #0F1D26; color: white; padding: 60px 50px; min-height: 260px; position: relative; overflow: hidden; }
...
<div class="page">
  ...
  <div class="content">
    ...
    <div class="section">
      <div class="section-header"><div class="section-number">1</div><div class="section-title">Brand Positioning Statement</div></div>
      <div class="positioning-box"><p>"${strategy.positioning_statement}"</p></div>
      ${strategy.tagline ? `<div class="card"><div class="card-label">Brand Tagline</div><div class="card-content" style="font-size:18px;font-weight:700;color:#0F1D26;font-family:'Poppins',sans-serif;">${strategy.tagline}</div></div>` : ''}
      ${strategy.elevator_pitch ? `<div class="card"><div class="card-label">Elevator Pitch</div><div class="card-content">${strategy.elevator_pitch}</div></div>` : ''}
    </div>` : ''}
    ${strategy.personas && Array.isArray(strategy.personas) && strategy.personas.length > 0 ? `
    <div class="section">
      <div class="section-header"><div class="section-number">2</div><div class="section-title">Customer Personas (${strategy.personas.length})</div></div>
      <div class="persona-grid">
        ...
        <div class="persona-card">
          ...
          ${p.traits?.length ? `<div class="trait-list">${p.traits.map((t: string) => `<span class="trait">${t}</span>`).join('')}</div>` : ''}
        </div>`).join('')}
      </div>
    </div>` : ''}
    ${strategy.key_messages?.length || strategy.tone_of_voice?.length ? `
    <div class="section">
      <div class="section-header"><div class="section-number">3</div><div class="section-title">Messaging Framework</div></div>
      ${strategy.key_messages?.length ? `<div class="card"><div class="card-label">Key Messages</div><ul class="message-list">${strategy.key_messages.map((m: string, i: number) => `<li class="message-item"><div class="message-num">${i+1}</div><div class="message-text">${m}</div></li>`).join('')}</ul></div>` : ''}
      ${strategy.tone_of_voice?.length ? `<div class="card"><div class="card-label">Tone of Voice</div><div class="tone-grid">${strategy.tone_of_voice.map((t: string) => `<span class="tone-tag">${t}</span>`).join('')}</div></div>` : ''}
    </div>` : ''}
    ${strategy.competitive_advantage ? `
    <div class="section">
      <div class="section-header"><div class="section-number">4</div><div class="section-title">Competitive Advantage</div></div>
      <div class="card"><div class="card-content">${strategy.competitive_advantage}</div></div>
      ${strategy.differentiation_points && Array.isArray(strategy.differentiation_points) ? (strategy.differentiation_points as any[]).map((p: any) => `<div class="diff-item"><div class="diff-dot"></div><div><div class="diff-label">${p.label}</div><div class="diff-desc">${p.desc}</div></div></div>`).join('') : ''}
    </div>` : ''}
  </div>
  <div class="footer">
    <div><strong>${businessName}</strong> · Brand Strategy · ${date}</div>
    <div>Generated by <strong>BrandOS</strong> · Ingweplex · brandosapp.vercel.app</div>
  </div>
</div>
