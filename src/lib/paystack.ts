// Paystack Payment Integration — Africa-wide
...
export async function initializeTransaction({
  ...
  reference: string; callbackUrl: string; metadata?: Record<string, unknown>
}) {
  const amountInKobo = Math.round(amount * 100)
  const response = await fetch(`${PAYSTACK_API_URL}/transaction/initialize`, {
    method: 'POST',
    headers: { 'Authorization': `Bearer ${SECRET_KEY}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ email, amount: amountInKobo, currency, reference, callback_url: callbackUrl, metadata: { ...metadata, cancel_action: callbackUrl } }),
  })
  ...
  return { authorization_url: data.data.authorization_url, access_code: data.data.access_code, reference: data.data.reference }
}
